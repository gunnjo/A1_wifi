...
dhdsdio_clkctl(dhd_bus_t *bus, uint target, bool pendok)
{
	...
	case CLK_NONE:
		...
		//&*&*&*JohWan1 del Disable watchdog timer setting if BCM4329 OOB support
		//dhd_os_wd_timer(bus->dhd, 0);
		//&*&*&*JohWan2 del Disable watchdog timer setting if BCM4329 OOB support
		break;
	}
#ifdef DHD_DEBUG
...
dhdsdio_release_dongle(dhd_bus_t *bus, osl_t *osh)
{
	...
	if (bus->sih) {
		dhdsdio_clkctl(bus, CLK_AVAIL, FALSE);
#if !defined(BCMLXSDMMC)
		dhdsdio_clkctl(bus, CLK_AVAIL, FALSE); //johnny+
