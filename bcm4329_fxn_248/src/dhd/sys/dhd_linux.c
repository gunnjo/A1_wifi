...
#if defined(CONFIG_WIFI_CONTROL_FUNC) // Ellis
...
static struct platform_driver wifi_device = {
	...
	.resume         = wifi_resume,
	.driver         = {
	.name   = "device_wifi", //.name   = "bcm4329_wlan",
	}
};
...
{
	printk("## Calling platform_driver_unregister\n");
...
char firmware_path[MOD_PARAM_PATHLEN]="/system/proprietary/wifi/rtecdc.bin";
//&*&*&*JohWan1 mod BCM4329 OOB support
#if defined(OOB_INTR_ONLY)
	char nvram_path[MOD_PARAM_PATHLEN]="/system/proprietary/wifi/nvram_oob.txt";
#else
	char nvram_path[MOD_PARAM_PATHLEN]="/system/proprietary/wifi/nvram.txt";
#endif
...
/* Use polling */
//&*&*&*JohWan1 mod Set polling eanble if BCM4329 OOB support
#if defined(OOB_INTR_ONLY)
	uint dhd_poll = TRUE;
#else
	uint dhd_poll = FALSE;
#endif
//&*&*&*JohWan2 mod Set polling eanble if BCM4329 OOB support
...
{
	/* 2010.09.03 Modified by JONO, disable the dhd_mmc_suspend flag. */
#if 0
	int ret = NOTIFY_DONE;
...
int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	uint roamvar = 1;
#endif /* CUSTOMER_HW2 */
	printk(">>> dhd_set_suspend, value: %d\n", value);
	DHD_TRACE(("%s: enter, value = %d in_suspend=%d\n", \
			__FUNCTION__, value, dhd->in_suspend));

	if (dhd && dhd->up) {
		printk("JMSG: dhd->in_suspend: %d, value: %d\n",dhd->in_suspend, value); //johnny+
		
		if (value && dhd->in_suspend) {

				/* Kernel suspended */
				DHD_TRACE(("%s: force extra Suspend setting \n", __FUNCTION__));
				printk("DHD: <Kernel suspended>\n");
	...
	if ((firmware_path != NULL) && (firmware_path[0] != '\0'))
     {
		strcpy(fw_path, firmware_path);
           printf("BCM4329: fw_path= %s\n", fw_path);
     }

	if ((nvram_path != NULL) && (nvram_path[0] != '\0'))
	  strcpy(nv_path, nvram_path);

     printf("BCM4329: nv_path= %s\n", nv_path);
...
// #if defined(CUSTOMER_HW2) && defined(CONFIG_WIFI_CONTROL_FUNC) // Ellis
...
//#endif		//Henry Li@2011.9.1
			...
			if (dhd->sysioc_pid >= 0) {
				...
				//&*&*&*JohWan1 force to kill process before suspend
				//wait_for_completion(&dhd->sysioc_exited);
				if (!wait_for_completion_timeout(&dhd->sysioc_exited, 2*HZ))
				{
					printk("dhd->sysioc_exited kill fail, force close it\n");
					up(&dhd->sysioc_sem);
					if (!wait_for_completion_timeout(&dhd->sysioc_exited, 2*HZ))
					{
						printk("dhd->sysioc_exited can't be killed \n");
					}
				}
				//&*&*&*JohWan2 force to kill process before suspend
			}
		...
		if (dhd->watchdog_pid >= 0)
		{
			...
			//printk("%s: wait_for_completion dhd->watchdog_exited \n", __FUNCTION__); 
			//&*&*&*JohWan1 force to kill process before suspend
			//wait_for_completion(&dhd->watchdog_exited);
			if (!wait_for_completion_timeout(&dhd->watchdog_exited, 2*HZ))
			{
				printk("dhd->watchdog_exited kill fail, force close it\n");
				up(&dhd->watchdog_sem);
				if (!wait_for_completion_timeout(&dhd->watchdog_exited, 2*HZ))
				{
					printk("dhd->watchdog_exited can't be killed \n");
				}
			}
			//&*&*&*JohWan2 force to kill process before suspend
		}
		...
		if (dhd->dpc_pid >= 0)
		{
			...
			//printk("%s: wait_for_completion dhd->dpc_exited \n", __FUNCTION__);
			//&*&*&*JohWan1 force to kill process before suspend
			//wait_for_completion(&dhd->dpc_exited);
			if (!wait_for_completion_timeout(&dhd->dpc_exited, 2*HZ))
			{
				printk("dhd->dpc_exited kill fail, force close it\n");
				up(&dhd->dpc_sem);
				if (!wait_for_completion_timeout(&dhd->dpc_exited, 2*HZ))
				{
					printk("dhd->dpc_exited can't be killed \n");
				}
			}
			//&*&*&*JohWan2 force to kill process before suspend
		}
